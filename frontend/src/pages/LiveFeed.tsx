...
export default function LiveFeed() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const response = await fetch('https://vigil-sdk-gateway.onrender.com/events', {
          method: 'POST',
          headers: {
          ...
          <div className="p-1.5 bg-slate-100 dark:bg-slate-900 rounded-lg flex gap-1">
            {(['ALL', 'CRITICAL', 'WARNING', 'LOW'] as const).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1 rounded text-[10px] font-black tracking-widest transition ${filter === f
          ...
          <AnimatePresence initial={false}>
            {filteredEvents.map((evt) => (
              <motion.div
                ...
                className={`bg-vigil-card rounded-xl border-l-4 p-4 shadow-sm flex items-center justify-between group hover:shadow-md transition-all ${evt.severity === 'CRITICAL' ? 'border-rose-500' : evt.severity === 'WARNING' ? 'border-amber-500' : 'border-teal-500'
                  }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${evt.severity === 'CRITICAL'
